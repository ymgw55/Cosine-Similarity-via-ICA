...
from .datasets.categorization import (fetch_AP, fetch_battig, fetch_BLESS,
                                      ...
                                      fetch_ESSLI_2c)
from .datasets.similarity import (fetch_MEN, fetch_MTurk, fetch_RG65, fetch_RW,
                                  fetch_SimLex999, fetch_TR9856, fetch_WS353)
...
def evaluate_ps_analogy(w, X, y, method="add", k=None, category=None, batch_size=100):
    """
    Simple method to score embedding using SimpleAnalogySolver
    ...
    ps = w.ps
    solver = PsAnalogySolver(w=w, method=method, batch_size=batch_size, k=k)
    p2y_pred = solver.predict(X)
    ...
    if category is not None:
        ...
        for p in ps:
            ...
            p2res[p] = pd.concat([pd.Series(results, name="accuracy"),
                              pd.Series(correct, name="correct"),
                              pd.Series(count, name="count")],
                            axis=1)
        return p2res
...
def evaluate_analogy_mrr(w, X, y, method="add", k=None, category=None, batch_size=100):
    """
    Simple method to score embedding using SimpleAnalogySolver
    ...
    solver = MRRAnalogySolver(w=w, method=method, batch_size=batch_size, k=k)
    y_rr = solver.predict(X, y)

    if category is not None:
        results = OrderedDict({"all": np.mean(y_rr)})
        count = OrderedDict({"all": len(y_rr)})
        for cat in set(category):
            results[cat] = np.mean(y_rr[category == cat])
            count[cat] = np.sum(category == cat)

        return pd.concat([pd.Series(results, name="mrr"),
                          pd.Series(count, name="count")],
                         axis=1)
    else:
        return np.mean(y_rr)
...
def evaluate_similarity(w, X, y):
    ...
    p = w.p
    scores = []
    normed_A = A / np.linalg.norm(A, axis=1, keepdims=True)  # shape: (3000, 300)
    normed_B = B / np.linalg.norm(B, axis=1, keepdims=True)  # shape: (3000, 300)
    for v1, v2 in zip(normed_A, normed_B):
        hadamard_product = v1 * v2
        # 成分の大きさに基づき、大きな p 個の値を取得
        top_p_components = np.partition(hadamard_product, -p)[-p:]
        score = np.sum(top_p_components)  # 選んだ成分の総和
        scores.append(score)

    return scipy.stats.spearmanr(scores, y).correlation


def return_cossim_and_toppsum(w, X, y):
    """
    Calculate Spearman correlation between cosine similarity of the model
    and human rated similarity of word pairs
    ...
    cossims = np.array([v1.dot(v2.T)/(np.linalg.norm(v1)*np.linalg.norm(v2)) for v1, v2 in zip(A, B)]).tolist()
    ...
    assert len(cossims) == len(scores)

    return cossims, scores
